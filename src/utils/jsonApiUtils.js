...
function createMergedStateObject(
    ...
    Object.keys(get(dataItem, 'relationships', {})).forEach(relationship => {
      if (dataItem.relationships[relationship].data) {
        ...
        if (!(dataItem.relationships[relationship].data instanceof Array)) {
          dataItem.relationships[relationship].data = [dataItem.relationships[relationship].data];
          normalise = singularCamel;
        }
        // no need for empty arrays
        if (dataItem.relationships[relationship].data.length) {
          state[pluralCamel(dataItem.type)][dataItem.id][normalise(relationship)] = {
        ...
        if (withTransform) {
          ...
          includes.forEach(includedKey => {
            if (includedKey === pluralCamel(relationship) || includedKey === singularCamel(relationship)) {
              // pluralize this so that we are always comparing lists to lists and not list
              includedKey = pluralCamel(relationship);

              state[pluralCamel(dataItem.type)][dataItem.id][normalise(relationship)] = uniqBy(
                dataItem.relationships[relationship].data,
                'id'
                ...
                if (
                  ...
                  Object.keys(state[includedKey][relationshipData.id]).length < 2
                ) {
  ...
  range(1, levelOfNesting + 1).forEach(() => {
    callAgain.forEach(item => {
      // withTransform
      const transformStateType = transformList[item.stateType] || transformList[pluralCamel(item.stateType)];
      const transformItemRelationship = transformList[item.relationship] || transformList[item.relationship];
      if (transformItemRelationship && state[pluralCamel(item.stateType)][item.stateId][transformItemRelationship]) {
        state[pluralCamel(item.stateType)][item.stateId][transformItemRelationship] = state[
          pluralCamel(item.stateType)
        ][item.stateId][transformItemRelationship].map(data => ({
          ...data,
          ...state[item.includedKey][data.id]
        }));
      } else if (transformStateType && state[transformStateType][item.stateId][transformItemRelationship]) {
        state[transformStateType][item.stateId][transformItemRelationship] = state[transformStateType][item.stateId][
          transformItemRelationship
        ].map(data => ({
          ...data,
          ...state[item.includedKey][data.id]
        }));
      } else if (state[pluralCamel(item.stateType)][item.stateId][item.relationship]) {
        if (!(state[pluralCamel(item.stateType)][item.stateId][item.relationship] instanceof Array)) {
          state[pluralCamel(item.stateType)][item.stateId][item.relationship] = Object.values(
            state[pluralCamel(item.stateType)][item.stateId][item.relationship]
          );
        }

        state[pluralCamel(item.stateType)][item.stateId][item.relationship] = state[pluralCamel(item.stateType)][
          item.stateId
        ][item.relationship].map(data => ({
          ...data,
          ...state[item.includedKey][data.id]
        }));
      } else if (state[item.stateType][item.stateId][item.relationship]) {
        if (!(state[item.stateType][item.stateId][item.relationship] instanceof Array)) {
          state[item.stateType][item.stateId][item.relationship] = Object.values(
            state[item.stateType][item.stateId][item.relationship]
          );
        }
        state[item.stateType][item.stateId][item.relationship] = state[item.stateType][item.stateId][
          item.relationship
        ].map(data => ({
          ...data,
          ...state[item.includedKey][data.id]
...
export function createShallowInclude(jsonApiResponse, entities, transformList = {}, levelOfNesting = 0) {
  // Create a consolidated state object with all the includedKeys
  let state = {};
  ...
  state = createMergedStateObject(included, entities, transformList, state, levelOfNesting);
  state = createMergedStateObject(data, entities, transformList, state, levelOfNesting);
  return state;
}
